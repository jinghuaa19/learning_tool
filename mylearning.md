...
逻辑回归的模型 是一个非线性模型，$sigmoid$ 函数，又称逻辑回归函数。但是它本质上又是一个线性回归模型，因为除去 $sigmoid$ 映射函数关系，其他的步骤，算法都是线性回归的。可以说，逻辑回归，都是以线性回归为理论支持的。只不过，线性模型，无法做到 $sigmoid$ 的非线性形式， $sigmoid$ 可以轻松处理0/1分类问题。

逻辑回归函数 $f(t) = { e^{t} \over {e^t + 1}} = {1 \over {1 + e^{-t}}}$ 表现了0,1分类的形式。
